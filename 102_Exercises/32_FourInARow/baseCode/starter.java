...
class starter {
	...
	// Any spot with an "O" in its place should be disregarded and continued to the next check
 	public static int checkVertical(String[][] board){
		...
		for(int column = 0; column < board[row].length; column++){
			if((row<board.length-1)&&(board[row][column].equals(board[row+1][column]))&&(board[row][column].equals("A"))){
				count++;
			}
		}
 		}
 	...
 	*/
 		...
 		for(int countRow  = 0; countRow < board.length; countRow++){
 			for(int countColumn = 0; countColumn < board[countRow].length; countColumn++){
 		if(board[countRow][countColumn].equals("A")){
 			countA++;
 		}
 			}
 		}
 		...
 		if(countA>3){
 			...
 			if(board[1][0].equals(board[2][0])){
 				count2++;
 			}
 			if(board[2][0].equals(board[3][0])){
 				count2++;
 			}
 			if(board[3][0].equals(board[4][0])){
 				count2++;
 			}
 			if(board[4][0].equals(board[5][0])){
 				count2++;
 			}
 			if(count2>3){
 				return 1;
 		...
 		while(countRow < board.length){
 		if(board[countRow][0].equals("B")){
 			countB++;
 		}
 		countRow++;
 		}

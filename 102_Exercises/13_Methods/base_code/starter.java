...
class starter {
	...
	public static boolean isLeapYear(int y){ //int y does not need to match the "year" int name
		if((y%4)==0){
			return true; //because isLeapYear is a boolean, we have to return a boolean
		}
		else{
			return false;
		}
	...
	public static int getDigitSum(int a){
		int answer = (a%10)+((a/10)%10)+((a/100)%10)+((a/1000)%10)+((a/10000)%10);
		return answer;
	}
	...
	public static void printIfConsecutive(int a, int b, int c){
		if((a+1==b)&&(b+1==c)){
			...
			System.out.println("It's NOT consecutive!");
		}
	}
	//end of my code

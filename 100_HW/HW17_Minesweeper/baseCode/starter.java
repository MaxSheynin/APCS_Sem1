...
class starter{
    ...
    public static void mapSetUp(int diff, int [][] bombProxGrid){
        ...
        for(int r = 0; r<bombProxGrid.length; r++){
            for(int c = 0; c<bombProxGrid[0].length; c++){
                bombProxGrid[r][c] = 0; // 0 = no bomb (yet!)
            }
        }
    ...
    */
    public static void addAdj(int r, int c, int [][] bombProxGrid){
        if(r-1>=0&&c-1>=0&&bombProxGrid[r-1][c-1]!=9){ 
            bombProxGrid[r-1][c-1] = bombProxGrid[r-1][c-1] + 1; 
        }
        if(r-1>=0&&bombProxGrid[r-1][c]!=9){ 
            bombProxGrid[r-1][c] = bombProxGrid[r-1][c] + 1; 
        }
        if(r-1>=0&&c+1<bombProxGrid[0].length&&bombProxGrid[r-1][c+1]!=9){ 
            bombProxGrid[r-1][c+1] = bombProxGrid[r-1][c+1] + 1; 
        }
        if(c-1>=0&&bombProxGrid[r][c-1]!=9){
            bombProxGrid[r][c-1] = bombProxGrid[r][c-1] + 1; 
        }
        if(c+1<bombProxGrid[0].length&&bombProxGrid[r][c+1]!=9){
            bombProxGrid[r][c+1] = bombProxGrid[r][c+1] + 1; 
        }
        if(r+1<bombProxGrid.length&&c-1>=0&&bombProxGrid[r+1][c-1]!=9){
            bombProxGrid[r+1][c-1] = bombProxGrid[r+1][c-1] + 1; 
        }
        if(r+1<bombProxGrid.length&&bombProxGrid[r+1][c]!=9){
            bombProxGrid[r+1][c] = bombProxGrid[r+1][c] + 1; 
        }
        if(r+1<bombProxGrid.length&&c+1<bombProxGrid[0].length&&bombProxGrid[r+1][c+1]!=9){ 
            bombProxGrid[r+1][c+1] = bombProxGrid[r+1][c+1] + 1; 
        }
    }
    ...
    */
    public static boolean move(int r, int c, int [][] bombProxGrid){
         if(r<bombProxGrid.length&&c<bombProxGrid[0].length&&bombProxGrid[r][c] == 9){
             return true;
         }
         return false;
    }
    ...
    */
    public static boolean checkWin(int [][] guessGrid, int [][] bombProxGrid){
        ...
        for(int r = 0; r<bombProxGrid.length; r++){
            for(int c = 0; c<bombProxGrid[0].length; c++){
                if(bombProxGrid[r][c]!=guessGrid[r][c]){
                    count++;
                }
            }
        }
